...
source("../../04_MSNA/2019_BGD_MSNAs/Functions/make_composite_indicators_bgd_msna_2019_mk.R")
...
msna_with_composite$household_composites
...
msna_indiv_with_composite<- msna_indiv_with_composite %>% left_join(msna_hh_with_composite %>% select(X_uuid,kobo_coords),by = c("X_submission__uuid"="X_uuid" ))
...
wash<-wash_hh_r3_2019_recoding(hh_data = wash_r3_hh_clean,
                               container_data = wash_r3_clean$Container_19June2019.csv,
                               individual_data = wash_r3_clean$Individual_19June2019.csv)

wash_hh<-wash$HH_dataset
wash_hh$time_wat %>% unique() %>% dput()
wash_hh<- wash_hh %>%
  mutate(
